...
struct _klp2 final : public ::faust::dsp {
	...
	void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		int iSlow4 = int(std::floor(0.5f * (fSlow1 + 1.0f)));
		int iSlow5 = iSlow0 + -1;
		for (int i0 = 0; i0 < count; i0 = i0 + 1) {
			iVec0[0] = 1;
			iRec0[0] = iRec0[1] + iSlow4 * (1 - iVec0[1]) + 2;
			int iTemp0 = iRec0[0] & iSlow5;
			float fTemp1 = float(iTemp0);
			float fTemp2 = float(input0[i0]);
			fVec1[IOTA0 & 65535] = fTemp2;
			output0[i0] = FAUSTFLOAT(fSlow2 * (1.0f - fSlow3 * fTemp1) * fVec1[(IOTA0 - std::min<int>(iSlow0, std::max<int>(0, iTemp0))) & 65535] * fTemp1);
			iRec1[0] = iRec1[1] + 2;
			int iTemp3 = iRec1[0] & iSlow5;
			float fTemp4 = float(iTemp3);
			output1[i0] = FAUSTFLOAT(fSlow2 * fTemp4 * (1.0f - fSlow3 * fTemp4) * fVec1[(IOTA0 - std::min<int>(iSlow0, std::max<int>(0, iTemp3))) & 65535]);
			iVec0[1] = iVec0[0];
			iRec0[1] = iRec0[0];
			IOTA0 = IOTA0 + 1;
			iRec1[1] = iRec1[0];
