...
Generates CI-ready exported project files for VST3 and AU modes by invoking the
HISE command line exporter and snapshotting the generated Binaries state after
each pass.
...
Options:
  ...
  --project-file PATH    Relative project XML/hip path. Default: XmlPresetBackups/Smut.xml
  ...
  --keep-active MODE     Leave live Binaries export as vst3|au. Default: au
  --snapshot-current MODE
                         Copy the current live Binaries tree to CIExports for
                         vst3|au|all without invoking HISE.
...
PROJECT_FILE="XmlPresetBackups/Smut.xml"
...
if [[ "$DO_VST3" -eq 0 && "$DO_AU" -eq 0 ]]; then
  echo "Nothing to do: all export modes are disabled." >&2
  exit 1
fi

if [[ "$KEEP_ACTIVE" != "vst3" && "$KEEP_ACTIVE" != "au" ]]; then
  echo "--keep-active must be 'vst3' or 'au'." >&2
  exit 1
fi

if [[ -n "$SNAPSHOT_CURRENT_MODE" && "$SNAPSHOT_CURRENT_MODE" != "vst3" && "$SNAPSHOT_CURRENT_MODE" != "au" && "$SNAPSHOT_CURRENT_MODE" != "all" ]]; then
  echo "--snapshot-current must be 'vst3', 'au', or 'all'." >&2
  exit 1
fi
...
if [[ -z "$SNAPSHOT_CURRENT_MODE" ]]; then
  ...
  normalise_app_bundle_bin HISE_BIN
  ...
  normalise_app_bundle_bin PROJUCER_BIN
